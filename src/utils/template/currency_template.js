...
function v_currencyformat( valueInCents, currencyCode ) {
	...
	if ( g_rgCurrencyData[currencyCode] ) {
		...
		let currencyReturn = IsCurrencySymbolBeforeValue( currencyCode ) ?
...
function GetCurrencyCode( currencyId ) {
	for ( let code in g_rgCurrencyData ) {
		if ( g_rgCurrencyData[code].eCurrencyCode == currencyId ) return code;
	}
	return 'Unknown';
}

function buildPrice(valueInCents, currencyId) {
	let currencyCode = GetCurrencyCode(currencyId);
	return v_currencyformat(valueInCents, currencyCode);
}
